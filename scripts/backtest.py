...
def _parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        default=60,
        help=(
            "Low Volatility 변동성 측정 영업일 수 (`--strategy-type=low-vol` 분기 전용). "
            "양수. 학술 표준 60."
        ),
    )
    parser.add_argument(
        "--rebalance-month-interval",
        type=int,
        default=3,
        help=(
...
def _run_low_volatility_pipeline(args: argparse.Namespace) -> None:
    """`--strategy-type=low-vol` 전용 파이프라인 — `compute_low_volatility_baseline` 호출.

    BacktestEngine 우회. universe 는 `_resolve_symbols(args.symbols, args.universe_yaml)`
    결과 사용. `--top-n` / `--lookback-days` / `--rebalance-month-interval` 로
    파라미터 주입.
    """
    universe = _resolve_symbols(args.symbols, args.universe_yaml)
    loader = _build_loader(args)
    ...
    config = LowVolBaselineConfig(
        ...
        top_n=args.top_n,
        rebalance_month_interval=args.rebalance_month_interval,
    )

    logger.info(
        "low_vol.start loader={l} from={s} to={e} universe={u} capital={c} "
        "lookback_days={lb} top_n={n} interval={i}",
        l=args.loader,
        s=args.start,
        e=args.end,
        u=len(universe),
        ...
        n=config.top_n,
        i=config.rebalance_month_interval,
    )

    try:
        result = compute_low_volatility_baseline(loader, config, args.start, args.end)
        ...
        args.output_markdown.write_text(
            _render_low_volatility_markdown(result, context, config=config),
            encoding="utf-8",
        )
        _write_metrics_csv(result.metrics, args.output_csv)
        _write_trades_csv(result.trades, args.output_trades_csv)
    ...
    logger.info(
        "low_vol.done trades={t} mdd={m} sharpe={s} verdict={v}",
        t=len(result.trades),
        m=_format_pct(result.metrics.max_drawdown_pct),
        s=_format_decimal(result.metrics.sharpe_ratio, 4),
        v=_low_volatility_verdict_label(result.metrics, daily_equity_len=len(result.daily_equity)),
    )


def _low_volatility_verdict_label(metrics: BacktestMetrics, *, daily_equity_len: int) -> str:
    """ADR-0022 게이트 1 + 게이트 3 자동 판정. 게이트 2 (DCA 대비 알파) 는 runbook 수동.
...
def _render_low_volatility_markdown(
    ...
    *,
    ...
    """Low Volatility 전용 Markdown 리포트 — ADR-0022 게이트 1·3 자동, 2 수동."""
    metrics = result.metrics
    verdict = _low_volatility_verdict_label(metrics, daily_equity_len=len(result.daily_equity))
    lines: list[str] = []

    lines.append("# Low Volatility 백테스트 리포트 (ADR-0019 Step F PR4)")
    lines.append("")
    lines.append(f"- 기간: `{context.start.isoformat()}` ~ `{context.end.isoformat()}`")
    lines.append(f"- universe 크기: {len(context.symbols)}")
    lines.append(f"- lookback_days: {config.lookback_days}")
    lines.append(f"- top_n: {config.top_n}")
    lines.append(f"- rebalance_month_interval: {config.rebalance_month_interval}")
    lines.append(f"- 시작 자본: {context.starting_capital_krw:,} KRW")
    lines.append(f"- 거래 수: {len(result.trades)}")
    lines.append("")
    lines.append(f"## ADR-0022 게이트 판정: **{verdict}**")
    ...
    lines.append(
        "- lookback_days 영업일 동안 lookback 미충족 — 첫 리밸런싱은 lookback 충족 후 "
        "주기 (분기) 도달 시 발화."
    )
    lines.append(
        "- 본 리포트의 MDD/Sharpe 는 단일 구간 결과. walk-forward 검증 (Phase 5) "
