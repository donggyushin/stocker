"""RSI 평균회귀 baseline 평가 함수 (ADR-0019 Step F PR5 — F5).

KOSPI 200 종목의 일봉 RSI(14) 평균회귀 전략 백테스트. ``RSIMRStrategy`` 가
universe 종목 close 를 누적하고 매 분봉마다 RSI 시그널 (Entry · stop_loss
Exit · take_profit Exit) 을 emit. 본 함수는 다중 lot 동시 보유 +
mark-to-market 평가를 담당.

설계 결정 — ``BacktestEngine`` 우회 (DCA · GoldenCross · Momentum · LowVol 동일 기조)
- ``BacktestEngine`` 은 단일 lot 가정 + ``force_close_at`` 청산 가정 — 다중
  종목 동시 보유 + 임의 시점 청산과 비호환.
- ``EntrySignal.take_price=0`` 마커 — 고정 익절 미사용. RSI 회귀로만 청산.

비용 계약 (DCA · GoldenCross · Momentum · LowVol 와 동일)
- 슬리피지: 시장가 0.1% 불리.
- 수수료: 매수·매도 대칭 0.015%.
- 거래세: 매도만 0.18%.

청산 reason 보존
- ``ExitSignal.reason`` (``stop_loss`` / ``take_profit``) 을 그대로
  ``TradeRecord.exit_reason`` 에 주입. 잔존 lot 가상 청산은 ``force_close``.
...
2. 매 분봉 처리:
   - 세션 경계 감지 (``bar_date != last_session_date``):
     ...
     b. ``last_session_date = bar_date``.
   - ``latest_close[bar.symbol] = bar.close`` 갱신.
   - ``strategy.on_bar(bar)`` 호출 → 시그널 즉시 처리.
3. 시그널 처리 — Exit 먼저 (cash 회수) → Entry (cash 분배):
   - Exit: ``ExitSignal.reason`` 보존 (``stop_loss`` / ``take_profit``).
   - Entry 자본 분배: ``alloc_per = cash_snapshot × position_pct / max_positions``
     (floor). ``qty = floor(alloc_per / entry_fill)`` (floor). qty=0 또는 잔액
     부족 시 skip.
4. 스트림 종료 시:
...
(RiskManager 미사용 — DCA · GoldenCross · Momentum · LowVol 와 동일).
...
class RSIMRBaselineConfig:
    """RSI 평균회귀 baseline 백테스트 파라미터.

    Raises:
        RuntimeError: 자본 비양수, 비율 음수, ``slippage_rate`` ``[0, 1)`` 범위
            위반. ``universe`` · ``rsi_period`` · ``oversold_threshold`` 등은
            ``RSIMRConfig`` 검증으로 위임 — 잘못된 값은 ``compute_rsi_mr_baseline``
            호출 시점에 ``RSIMRConfig`` ``__post_init__`` 가 ``RuntimeError`` 전파.
    """
...
def compute_rsi_mr_baseline(
    ...
    """RSI 평균회귀 시뮬레이션 → ``BacktestResult``.

    Args:
        loader: ``BarLoader`` Protocol 구현체. 보통 ``DailyBarLoader``.
        config: RSI 평균회귀 baseline 파라미터.
        start, end: 백테스트 구간 (경계 포함).

    Raises:
        RuntimeError: ``start > end``, ``RSIMRConfig`` 검증 위반.
    ...
    strategy = RSIMRStrategy(rsi_cfg)
    ...
    def _process_signals(signals: list[Signal]) -> None:
        ...
        for sig in exits:
            ...
            if lot is None:
                logger.debug("RSI-MR exit skip: 보유 없음 (sym={s})", s=sig.symbol)
                continue
        ...
        if not entries:
            return
        # 동시 진입 시 각 entry 가 (남은 cash / 남은 slot) 만큼 받도록 per-signal 재계산.
        # max_positions=2, 1 lot 보유 중 새 entry → 남은 cash 전부 1 슬롯에 배분.
        for sig in entries:
            if sig.symbol in active_lots:
                logger.debug("RSI-MR entry skip: 이미 보유 (sym={s})", s=sig.symbol)
                continue
            available_slots = config.max_positions - len(active_lots)
            if available_slots <= 0:
                logger.debug("RSI-MR entry skip: max_positions 도달 (sym={s})", s=sig.symbol)
                continue
            alloc_per = Decimal(cash) * config.position_pct / Decimal(available_slots)
...
def _attempt_buy(
    ...
    if entry_fill <= 0:
        logger.debug("RSI-MR skip: entry_fill 비양수 ({})", entry_fill)
        return cash, None

    qty = int(alloc_per_position / entry_fill)
    if qty <= 0:
        logger.debug(
            "RSI-MR skip: alloc 적용 후 qty=0 (alloc={}, fill={})",
            alloc_per_position,
            entry_fill,
        )
        return cash, None
    ...
    if total_cost > cash:
        logger.debug("RSI-MR skip: 잔액 부족 (need={}, have={})", total_cost, cash)
        return cash, None
    ...
    logger.info(
        "RSI-MR buy: {s} qty={q} entry={p} cost={c}",
        s=signal.symbol,
        q=qty,
        p=entry_fill,
        c=total_cost,
...
def _execute_exit(
    ...
    """``ExitSignal`` 처리. ``(new_cash, TradeRecord)``. ``signal.reason`` 보존."""
    ...
    logger.info(
        "RSI-MR sell: {s} reason={r} qty={q} exit={p} pnl={pnl}",
        s=signal.symbol,
        r=signal.reason,
        q=lot.qty,
        p=exit_fill,
...
def _hypothetical_exit(
    ...
    """스트림 종료 시 잔존 lot 가상 청산. ``exit_reason='force_close'``."""
...
def _compute_metrics(
    *,
    ...
    """``BacktestEngine._compute_metrics`` 와 동일 산식 — DCA · GoldenCross · Momentum · LowVol 와 동일."""  # noqa: E501
