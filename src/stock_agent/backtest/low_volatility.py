"""Low Volatility baseline 평가 함수 (ADR-0019 Step F PR4 — F4).

KOSPI 200 종목의 분기 (또는 ``rebalance_month_interval`` 개월) 리밸런싱 저변동성
전략 백테스트. ``LowVolStrategy`` 가 universe close 를 누적하고 주기 변경
시점에 ``ExitSignal`` · ``EntrySignal`` 다중 emit. 본 함수는 다중 lot 동시
보유 + mark-to-market 평가를 담당.

설계 결정 — ``BacktestEngine`` 우회 (DCA · GoldenCross · Momentum 와 동일 기조)
- ``BacktestEngine`` 은 단일 lot 가정 + ``force_close_at`` 청산 가정 — 다중
  종목 동시 보유 + 주기 단위 리밸런싱과 비호환.
- ``EntrySignal.stop_price=0 / take_price=0`` 마커 인식 — 손익절 판정 건너뜀.

비용 계약 (DCA · GoldenCross · Momentum 과 동일)
- 슬리피지: 시장가 0.1% 불리.
- 수수료: 매수·매도 대칭 0.015%.
- 거래세: 매도만 0.18%.
...
(RiskManager 미사용 — DCA · GoldenCross · Momentum 과 동일).
...
class LowVolBaselineConfig:
    """Low Volatility baseline 백테스트 파라미터.

    Raises:
        RuntimeError: 자본 비양수, 비율 음수, ``slippage_rate`` ``[0, 1)`` 범위
            위반. ``universe`` · ``lookback_days`` · ``top_n`` 등은
            ``LowVolConfig`` 검증으로 위임 — 잘못된 값은 ``compute_low_volatility_baseline``
            호출 시점에 ``LowVolConfig`` ``__post_init__`` 가 ``RuntimeError`` 전파.
    """
...
def compute_low_volatility_baseline(
    loader: BarLoader,
    config: LowVolBaselineConfig,
    start: date,
    end: date,
) -> BacktestResult:
    """Low Volatility 시뮬레이션 → ``BacktestResult``.

    Args:
        loader: ``BarLoader`` Protocol 구현체. 보통 ``DailyBarLoader``.
        config: Low Volatility baseline 파라미터.
        start, end: 백테스트 구간 (경계 포함).

    Raises:
        RuntimeError: ``start > end``, ``LowVolConfig`` 검증 위반.
    ...
    low_vol_cfg = LowVolConfig(
        universe=config.universe,
        lookback_days=config.lookback_days,
        top_n=config.top_n,
        rebalance_month_interval=config.rebalance_month_interval,
        position_pct=config.position_pct,
    )
    strategy = LowVolStrategy(low_vol_cfg)
    ...
    def _process_signals(signals: list[Signal]) -> None:
        ...
        for sig in exits:
            ...
            if lot is None:
                logger.debug("LowVol exit skip: 보유 없음 (sym={s})", s=sig.symbol)
                continue
        ...
        for sig in entries:
            if sig.symbol in active_lots:
                logger.debug("LowVol entry skip: 이미 보유 (sym={s})", s=sig.symbol)
                continue
...
def _attempt_buy(
    ...
    if entry_fill <= 0:
        logger.debug("LowVol skip: entry_fill 비양수 ({})", entry_fill)
        return cash, None

    qty = int(alloc_per_position / entry_fill)
    if qty <= 0:
        logger.debug(
            "LowVol skip: alloc 적용 후 qty=0 (alloc={}, fill={})",
            alloc_per_position,
            entry_fill,
        )
        return cash, None
    ...
    if total_cost > cash:
        logger.debug("LowVol skip: 잔액 부족 (need={}, have={})", total_cost, cash)
        return cash, None
    ...
    logger.info(
        "LowVol buy: {s} qty={q} entry={p} cost={c}",
        s=signal.symbol,
        q=qty,
        p=entry_fill,
        c=total_cost,
...
def _execute_exit(
    ...
    logger.info(
        "LowVol sell: {s} qty={q} exit={p} pnl={pnl}",
        s=signal.symbol,
        q=lot.qty,
        p=exit_fill,
        pnl=net_pnl,
...
def _compute_metrics(
    *,
    ...
    """``BacktestEngine._compute_metrics`` 와 동일 산식 — DCA / GoldenCross / Momentum 과 동일."""
