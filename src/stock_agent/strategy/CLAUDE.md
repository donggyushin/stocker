...
`golden_cross.py` 공개 심볼 (`strategy/golden_cross.py` — `__init__.py` 미재노출, 직접 import):

`GoldenCrossConfig`, `GoldenCrossStrategy`

`momentum.py` 공개 심볼 (`strategy/momentum.py` — `__init__.py` 미재노출, 직접 import):
...
## `golden_cross.py` — GoldenCrossStrategy (Step F PR2)

ADR-0019 Step F 복구 로드맵 두 번째 전략 후보. ADR-0022 게이트 비교 대상으로 도입 (PR2, 2026-05-02). ADR-0022 게이트 판정: PASS (단, 단일 trade caveat 적용).

`strategy/__init__.py` 에 재노출하지 않음 — 소비자 `compute_golden_cross_baseline` (`backtest/golden_cross.py`) 이 직접 import.

### `GoldenCrossConfig` (`@dataclass(frozen=True, slots=True)`)

| 필드 | 기본값 | 설명 |
|---|---|---|
| `target_symbol` | `"069500"` | 매수 대상 종목코드 (6자리 숫자, KODEX 200 기본) |
| `sma_period` | `200` | 단순이동평균 기간 (일봉 기준, 양수 필수) |
| `position_pct` | `Decimal("1.0")` | 자본 투입 비율 (0 초과 ~ 1.0 이하) |
...
### `GoldenCrossStrategy` — 200일 SMA 추세 추종 전략

- `Strategy` Protocol 구현체 (`on_bar`, `on_time`).
- **의미론**: 매일 종가 기준 SMA(sma_period) 계산. 종가 > SMA(cross-up) 시 `EntrySignal`, 종가 < SMA(cross-down) 시 `ExitSignal`. lookback 부족(일봉 수 < sma_period) 시 시그널 없음.
- `on_time` 은 항상 빈 리스트 반환 — 강제청산 없음 (일봉 전략, 장 마감 후 판정).
- **1일 1회 판정**: 일봉 기준 동작.
- **세션 경계 자동 리셋**: `bar.bar_time.date()` 변경 감지 시 해당 세션 시그널 판정.
- **입력 검증 (`RuntimeError` 전파)**: aware datetime 강제 (naive → 거부), symbol 6자리 숫자 정규식.

### 설계 특성

- 다중 lot 누적·mark-to-market 평가는 `backtest/golden_cross.py` 의 `compute_golden_cross_baseline` 이 담당 (`BacktestEngine` 우회).
- `BacktestEngine` 에 직접 주입 시 단일 lot 가정 + force_close 기반 청산과 충돌 — `compute_golden_cross_baseline` 경유가 필수.

### 테스트 현황 (GoldenCrossStrategy)

pytest **48 케이스 green** (`tests/test_strategy_golden_cross.py`). 외부 목킹 불필요 — 순수 로직.

| 그룹 | 내용 |
|---|---|
| Config 검증 | 심볼 정규식·sma_period 양수·position_pct 범위 |
| SMA 계산 | 200 종가 평균 정확성, lookback 부족 시 시그널 보류 |
| 진입 시그널 | cross-up 시 EntrySignal, 보유 중 추가 진입 X |
| 청산 시그널 | cross-down 시 ExitSignal |
| 입력 검증 | naive datetime 거부, symbol 포맷 |

관련 테스트 파일: `tests/test_strategy_golden_cross.py`.

---

## `momentum.py` — MomentumStrategy (Step F PR3)

ADR-0019 Step F 복구 로드맵 세 번째 전략 후보. Cross-sectional 모멘텀 (Jegadeesh-Titman 1993) 월별 리밸런싱. ADR-0022 게이트 판정: FAIL (게이트 2 — DCA 대비 알파 -36.96%p).

`strategy/__init__.py` 에 재노출하지 않음 — 소비자 `compute_momentum_baseline` (`backtest/momentum.py`) 이 직접 import.

### `MomentumConfig` (`@dataclass(frozen=True, slots=True)`)

| 필드 | 기본값 | 설명 |
|---|---|---|
| `universe` | 필수 | `tuple[str, ...]` — 평가 대상 종목 (1개 이상, 6자리 숫자 정규식, 중복 금지) |
| `lookback_months` | `12` | 모멘텀 측정 lookback 기간 (월 단위, 양수 필수). `lookback_days = lookback_months × 21` 영업일 근사 |
| `top_n` | `10` | 리밸런싱 시 보유 종목 수 (1 이상). 사용자 명시 값이 `len(universe)` 초과 시 `RuntimeError`; 기본값 10 은 작은 universe 테스트 편의로 허용 |
| `rebalance_day` | `1` | 월 중 N번째 영업일에 리밸런싱 (1~28). 현재 구현은 매월 첫 분봉 트리거로 단순화 |
| `position_pct` | `Decimal("1.0")` | 자본 투입 비율 (0 < pct ≤ 1) |

`__post_init__` 검증 (위반 시 `RuntimeError` — 다른 모듈과 동일 기조):
- `universe` 빈 tuple / 정규식 위반 / 중복
- `lookback_months > 0`
- `top_n >= 1`, `top_n != _DEFAULT_TOP_N` 일 때만 `top_n <= len(universe)` 강제
- `1 <= rebalance_day <= 28`
- `0 < position_pct <= 1`

### `MomentumStrategy` — Cross-sectional 모멘텀 상태 머신

- `Strategy` Protocol 구현체 (`on_bar`, `on_time`).
- **의미론**: universe 종목의 일봉 close 를 누적. 월 변경 시 (매월 `rebalance_day` 번째 영업일) lookback 기간 누적 수익률 계산 → 상위 `top_n` 종목 보유, 하위 종목 청산.
- `on_bar` 에서 일봉 close 버퍼 업데이트. 월 변경 리밸런싱 트리거 시 기존 보유 종목 `ExitSignal` + 신규 top-N 종목 `EntrySignal` 다중 emit.
- `on_time` 은 항상 빈 리스트 반환 — 강제청산 없음 (월별 리밸런싱 전략).
- **lookback 부족 시**: 누적 일봉 수가 `lookback_months × 21` 미만이면 리밸런싱 스킵.
- **non-universe 종목 가드**: universe 에 없는 symbol 은 `on_bar` 에서 조용히 무시 (multi-symbol 환경에서 다른 전략 bar 혼입 방어).
- **세션 경계 자동 리셋**: 월 변경 감지는 `bar.bar_time.date()` 의 월 필드 변경으로 판정.
- **입력 검증 (`RuntimeError` 전파)**: aware datetime 강제 (naive → 거부).

### 설계 특성

- 다중 lot 동시 보유 + mark-to-market 평가는 `backtest/momentum.py` 의 `compute_momentum_baseline` 이 담당 (`BacktestEngine` 우회).
- `BacktestEngine` 의 단일 lot 가정 + force_close_at 청산과 충돌 — `compute_momentum_baseline` 경유가 필수.
- DCA·GoldenCross 와 동일 비용 계약 (slip 0.1% / commission 0.015% / sell tax 0.18%).

### 테스트 현황 (MomentumStrategy)

pytest **47 케이스 green** (`tests/test_strategy_momentum.py`). 외부 목킹 불필요 — 순수 로직.
