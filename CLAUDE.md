...
- **Phase 1 진행 중 — 브로커 래퍼 + 데이터 파이프라인** (첫 산출물 완료 2026-04-19)
  ...
  - `src/stock_agent/data/universe.py` + `config/universe.yaml` — 완료. KOSPI 200 유니버스 YAML 하드코딩. pykrx 지수 API(`get_index_portfolio_deposit_file` 등) 와 KIS Developers 모두 인덱스 구성종목 API 미제공으로 수동 관리. 분기 리밸런싱 때 운영자 갱신. 현재 대형주 10개 seed, 실전 투입 전 200개 전량 채움 필수.
  ...
  - pytest 68건 green (test_config 5 + test_kis_client 15 + test_safety 5 + test_rate_limiter 18 + test_historical 14 + test_universe 11).
  - 의존성 추가: `pykrx 1.2.7` (+ transitive: pandas, numpy, matplotlib 등), `pyyaml 6.0.3`.

- **다음 작업 (Phase 1 잔여)**
  1. `src/stock_agent/data/realtime.py` — 장중 분봉 폴링 또는 WebSocket 실시간 체결가
