...
- **Phase 3 첫 산출물 — Executor 단독 (코드·테스트 레벨) 완료 (2026-04-21)**
  ...
  - 흐름: `step(now)` = reconcile → 분봉 처리(`on_bar`) → 시각 처리(`on_time`). EntrySignal 은 `evaluate_entry` → `submit_buy` → `_wait_fill` → `record_entry`. ExitSignal 은 `submit_sell` → `_wait_fill` → PnL 계산 → `record_exit`. `_wait_fill` 은 `get_pending_orders` 폴링, timeout → `ExecutorError`. `_with_backoff` 는 `KisClientError` 한정 지수 백오프 (기본 100→200→400 ms, 총 4회 시도) 후 `ExecutorError` 승격.
  - 재동기화: 매 step 마다 `get_balance().holdings` ↔ `risk_manager.active_positions` 비교. 불일치 시 `_halt = True` + `logger.critical`. 자동 복구 없음 — 운영자 개입 + 다음 `start_session` 으로 리셋.
  - pytest **542 → 605건 green** (`tests/test_executor.py` 63건 신규). 회귀 0건. 의존성 추가 없음.
  - 미완료: `main.py` (APScheduler 진입점) · `monitor/notifier.py` (텔레그램 알림) · `storage/db.py` (SQLite 영속화) · `broker/` 체결조회 API 통합 (실체결가 정확도) — 모두 후속 PR. **Phase 3 PASS 선언은 모의투자 연속 10영업일 무중단 운영 후.**
...
- **다음 작업**
  ...
  - **Phase 3 후속 PR**: `execution/executor.py` 완료 (2026-04-21) · `main.py` (APScheduler 통합, 미착수) · `monitor/notifier.py` (텔레그램 알림, 미착수) · `storage/db.py` (SQLite 체결 기록, 미착수) 순서로 진행.
