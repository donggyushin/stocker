...
Python 기반 한국주식 **데이트레이딩** 자동매매 시스템. 한국투자증권 KIS Developers API + Opening Range Breakout(ORB) 전략 + 100~200만원 초기 자본. **paper 주문 + live 시세 하이브리드 키** 구조 (KIS paper 도메인에 시세 API 없음 — 시세는 별도 실전 APP_KEY 로 실전 도메인 호출). 현재 **Phase 1 코드 완료 / Phase 2 착수 대기** (운영자 `.env` 실전 키 3종 설정 + `scripts/healthcheck.py` 4종 통과 확인 후 Phase 1 PASS 선언).
...
- **Phase 1 진행 중 — 브로커 래퍼 + 데이터 파이프라인** (첫 산출물 완료 2026-04-19)
  ...
  - `src/stock_agent/broker/rate_limiter.py` — 완료. 주문 경로 전용 `OrderRateLimiter`(기본 2 req/s + 최소 간격 350 ms, 단일 프로세스). 조회 경로는 python-kis 내장에 위임. pytest 43/43 green.
  ...
  - pytest 115건 green (test_config 11 + test_kis_client 15 + test_safety 18 + test_rate_limiter 18 + test_historical 14 + test_universe 11 + test_realtime 24 + 기타 회귀 없음).
  - 의존성 추가: `pykrx 1.2.7` (+ transitive: pandas, numpy, matplotlib 등), `pyyaml 6.0.3`.

- **다음 작업 (Phase 1 → Phase 2 전이)**
  1. 운영자 `.env` 에 실전 키 3종(`KIS_LIVE_APP_KEY`, `KIS_LIVE_APP_SECRET`, `KIS_LIVE_ACCOUNT_NO`) 기입 + KIS Developers 포털에서 사용 IP 화이트리스트 등록 (HTS_ID 는 paper/실전 공유).
  2. `uv run python scripts/healthcheck.py` 4종 통과 확인 (1: KIS 토큰, 2: 잔고 조회, 3: 텔레그램, 4: 삼성전자 현재가 — 실전 키 미설정 시 4번 SKIP).
  3. 4종 통과 → Phase 1 PASS 선언 → Phase 2 착수.
