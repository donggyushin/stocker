...
- **복구 로드맵 진행 상황**:
  ...
  - **Step D** — 전략 파라미터 구조 변경 (OR 윈도·force_close_at·재진입·일 N 진입). **Step C FAIL 확인 → 진입 단계.**
    ...
    - **D2 — force_close_at 스터디 (코드 단계 완료): 백테스트 미실행.** `step_d2_grid` 3×4×4 = 48 조합. `force_close_at` ∈ {14:50, 15:00, 15:20}. `--grid step-d2` 플래그 추가. 운영자 실행 후 ADR-0019 게이트 판정 예정.
...
- **테스트 카운트**: pytest **1487 passed, 4 skipped** (Step D2 PR 기준 — 이전 1478 + 9건 신규: `test_sensitivity.py` `TestStepD2Grid` 9건 + `test_sensitivity_cli.py` `TestGridFlag` `step-d2` 분기 1건 포함).
