...
- **Phase 1 진행 중 — 브로커 래퍼 + 데이터 파이프라인** (첫 산출물 완료 2026-04-19)
  ...
  - `src/stock_agent/data/` 패키지 신설 — `HistoricalDataStore` + SQLite 캐시(일봉, KOSPI 200 구성종목). 모듈 세부(공개 API, 캐시 정책, 에러 정책, 테스트 정책)는 [src/stock_agent/data/CLAUDE.md](./src/stock_agent/data/CLAUDE.md) 참조.
  - pytest 60건 green (test_config 5 + test_kis_client 15 + test_safety 5 + test_rate_limiter 18 + test_historical 17).
  - 의존성 추가: `pykrx 1.2.7` (+ transitive: pandas, numpy, matplotlib 등).
