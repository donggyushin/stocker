...
- **복구 로드맵 진행 상황**:
  ...
  - **Step D** — 전략 파라미터 구조 변경 (OR 윈도·force_close_at·재진입·일 N 진입). **Step C FAIL 확인 → 현재 진입 단계. D1 — OR 윈도 스터디 (`step_d1_grid`, 48 조합, `--grid step-d1` 플래그) 코드 단계 완료. 백테스트 미실행 — 운영자 실행 + 결과 댓글 후 D1 채택/미채택 판정 예정.**
...
- **테스트 카운트**: pytest **1478 passed, 4 skipped** (Step D1 PR 기준 — 이전 1408 + 70건 신규: `test_sensitivity.py` `TestStepD1Grid` 8건 + `test_sensitivity_cli.py` `TestGridFlag` 3건 + 기타 추가분).
