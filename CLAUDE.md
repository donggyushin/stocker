...
- **다음 작업**
  ...
  - **Issue #71 close 대기**: `KisMinuteBarLoader` 장시간 hang 방지 코드·테스트 레벨 완료 (2026-04-23). `http_timeout_s` + `http_max_retries_per_day` kwarg 추가, `scripts/backfill_minute_bars.py` CLI 옵션 동기화. 잔여: 운영자 실 백필 1회 hang 없이 완주 검증 → Issue #71 댓글 첨부 후 close.
