...
**한 줄 진행도**: Phase 1 PASS · Phase 2 진행 중 (1차 백테스트 FAIL → ADR-0019 복구 로드맵 Step F PR1 PASS · PR2 PASS). Phase 3 코드 산출물 완료 상태로 보존, 진입 금지.
...
- **복구 로드맵 진행 상황**:
  ...
  - **Step F** — 가설 풀 확장 (ADR-0021 결정, ADR-0022 게이트 적용). 진입 중. 게이트 재정의: MDD > -25% · DCA baseline 대비 양의 알파 · 연환산 샤프 > 0.3 (세 조건 동시). 상세 진행 계획: `docs/step_f_strategy_pool_plan.md`.
    ...
    - **PR2 (F2 Golden Cross) 완료 — PASS (2026-05-02)**. MDD -20.52% · Sharpe 2.2753 · 총수익률 +182.36% mark-to-market (시작 자본 2,000,000 KRW, KODEX 200 069500, 2024-06-01 ~ 2026-04-21). DCA 대비 알파 +130.86%p. 단, trades=1 (통계 신뢰도 낮음) + 데이터 plausibility caveat (069500 가격 2.93× 이상 급등 — 수정주가 검증 필요). 런북: `docs/runbooks/step_f_golden_cross_2026-05-02.md`. 후속 PR (F3 Cross-sectional 모멘텀 등) 진행 대기.
- **Phase 3 진입 금지 (ADR-0019)**: 게이트 통과 전까지 `main.py` 모의투자 무중단 운영 계획 전면 보류. `execution/`·`main.py`·`monitor/`·`storage/` 코드 산출물은 보존 — 복구 후 그대로 재사용.
- **테스트 카운트**: pytest **1830 collected** (Step F PR2 기준 — Step F PR1 1749 + PR2 신규 81건: `test_strategy_golden_cross.py` 48 + `test_backtest_golden_cross.py` 33).
