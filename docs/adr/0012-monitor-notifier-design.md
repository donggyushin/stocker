...
PR #18 머지 전 코드 리뷰 피드백 반영:

- **C1 — `ExitEvent.reason` 타입 좁힘**: `str` → `ExitReason` (`Literal["stop_loss","take_profit","force_close"]`). 원천 `ExitSignal.reason` 및 docstring 계약과 타입이 일치하도록 수정. 런타임 동작 불변, 정적 타입 계약만 강화.
- **I4 — `notify_daily_summary` `halted` 중복 출력 제거**: `DailySummary.halted` 는 RiskManager 서킷브레이커와 Executor `_halt` 의 합집합 단일 필드이므로 텔레그램 본문에 두 줄로 찍으면 정보 이득이 없고 "두 상태가 분리 집계된다" 는 오해만 유발. 한 줄(`매매 중단(halted)={yes|no}`) 로 통합.

나머지 리뷰 피드백(I1/I2/I3/I5/I6 — 운영 안정성·테스트 확장 5건) 은 후속 PR 로 이관. 항목 목록은 `plan.md` Phase 3 후속 작업 섹션 참조.
