...
- 도입 PR: TBD (Issue #35).
- [2026-04-23] Issue #52 대응 (PR #56): `_parse_row` 원인 카테고리별 dedupe 로깅 + 날짜 단위 skip counts 요약 warning + M2 경보에 첫 행 `sorted(keys)` 동봉. 카테고리: `missing_date_or_time` / `date_mismatch` / `invalid_price` / `invalid_volume` / `malformed_bar_time`. 진단 스크립트 `scripts/debug_kis_minute.py` 신규 (`--include-values` opt-in). `scripts/backfill_minute_bars.py` `--throttle-s` 기본값 0.0 → 0.2 (KIS 실전 시세 API 20 req/s 한도 대비 5 req/s 수준 유지). 실 응답 계약은 2026-04-23 11:03 KST 평일 진단 (`005930`) 으로 7키 (`stck_bsop_date` · `stck_cntg_hour` · `stck_oprc` · `stck_hgpr` · `stck_lwpr` · `stck_prpr` · `cntg_vol`) + 여분 `acml_tr_pbmn` 1필드 확정. `response.__data__` 경유 raw dict, `first_row_type=dict` — 코드 가정 100% 일치.
